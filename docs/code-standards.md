...
- A client-ticking region (e.g. the countdown) carries `aria-live="polite" aria-atomic="true"` on the wrapper that changes, so assistive tech announces the new values without a full page re-read (`event-countdown.tsx`).
- A toggle that shows/hides a menu (e.g. the FAB) carries `aria-expanded={state}` and `aria-controls` pointing at the menu's `id`. The menu stays mounted at all times — a Tailwind `hidden` class toggles visibility, never conditional unmount — so `aria-controls` always resolves to a real element and `display:none` alone keeps the inactive half out of both layout and the tab order (`fab-widget.tsx`).
...
- A flaky external call under parallel E2E workers (e.g. Supabase `verifyOtp` JWT `iat` collisions under concurrent test workers) gets a bounded `withRetry(fn, maxAttempts, delayMs)` helper (`e2e/support/with-retry.ts`) rather than a longer fixed timeout or a retried whole test — extracted out of `seed-session.ts` to keep it under the 200-line file rule.
...
- Every locale-bearing shell — including a route-group's own header, not only the shared `(site)` shell — must wire its `LanguageDropdown`/similar to a real `onSelectLocale`. A rendered dropdown with no action wired is a silent no-op defect, not a cosmetic gap: `(auth)/layout.tsx` originally rendered `LoginHeader` with a hardcoded `locale="vi"` and no `onSelectLocale`, so clicking EN on `/login` did nothing.
