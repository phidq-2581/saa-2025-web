...
4. **`login.json` is partially wired (Phase 07)**: `login-hero.tsx` and `login-footer.tsx` now read `heroSubtitle`/`heroTagline`/`footerCopyright` via `getTranslations("login")`. `login-header.tsx` still hard-codes its logo `alt` text as a literal (locale-invariant either way, so not a visible bug, just an unwired catalog key), and `login-error-notice.tsx` still hard-codes its Vietnamese OAuth-failure copy regardless of locale despite an `errorMessage` key existing in both `login.json` catalogs — the latter is a genuine EN-locale gap, left for Phase 07b (`plans/clarifications.md` § Session 2026-08-30) alongside the rest of the Group 3 body-copy conversion.
