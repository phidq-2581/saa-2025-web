...
**Source:** `src/components/homepage/hero-section.tsx:1-112` — FR-001, keyvisual hero + wordmark + CTA pair; background image/gradient layers use `-z-10` so they can bleed under `RootFurtherBlock` without adding to document flow height. Renders `EventCountdownLive` directly (Phase 07) rather than receiving a `remaining` prop itself. Reads copy via `useTranslations("home")` since Phase 07b.
...
**Source:** `src/components/homepage/event-countdown.tsx:1-76` — FR-002, BR-001_CountdownZeroPadding: presentational tiles only, receives the already-computed `remaining` prop and never touches the clock itself. Reads its coming-soon label via `useTranslations("home")` since Phase 07b.
**Source:** `src/components/homepage/event-info.tsx:1-44` — FR-001, event info line; renders the spec-CSV copy ("18h30" / "Nhà hát nghệ thuật quân đội"), not the stale Figma canvas text (Group 3 checkpoint decision — spec CSV wins). Reads copy via `useTranslations("home")` since Phase 07b.
...
**Source:** `src/components/homepage/root-further-block.tsx:1-58` — FR-004, static Root Further theme description. Reads copy via `useTranslations("home")` since Phase 07b.
**Source:** `src/components/homepage/award-grid.tsx:1-90` — FR-005/FR-006, BR-006/BR-007: 6-card grid, each card's `href` is `/he-thong-giai#{slug}` (falls back to no hash when a category has no slug). Reads copy via `useTranslations("home")` since Phase 07b.
**Source:** `src/components/homepage/award-card.tsx:1-80` — FR-005/FR-006: one `<a>` wraps the whole card body so thumbnail/title/'Chi tiết' share a single navigation target (BR-006). Reads copy via `useTranslations("home")` since Phase 07b.
**Source:** `src/components/homepage/kudos-promo.tsx:1-79` — FR-007, BR-008_AboutKudosDeferredAffordance: inert "Chi tiết" CTA (`type="button" aria-disabled="true" tabIndex={-1}`). Reads copy via `useTranslations("home")` since Phase 07b.
