...
**Source:** `src/components/awards/award-category-nav.tsx:1-123` — FR-002/FR-003, BR-001/BR-002, SM-001: `useSyncExternalStore` hash read (empty on server and on the client's first render, so the real hash resync happens post-mount — same hydration-safety pattern as `src/lib/countdown/use-countdown.ts`); `clickedSlug` overrides the hash-derived slug as the single `activeSlug` source of truth; exactly one `aria-current="true"` at a time. Phase 07: `aria-current` is mirrored onto the outer `<li data-testid="award-nav-item">` as well as the inner `<button>` — `e2e/integration-locale-countdown.spec.ts`'s award-card-click assertion (test 6, BR-006/BR-002) resolves the compound selector `[data-testid="award-nav-item"][data-slug="..."]` to the `<li>` and reads `aria-current` directly on it, a check the button-only attribute alone (still asserted unchanged by `award-system.spec.ts`'s descendant queries) can't satisfy; `aria-current` on a non-interactive wrapper is valid ARIA.
