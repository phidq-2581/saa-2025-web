...
/**
 * Inline icon components for the compose modal (code-rules 2a). The
 * downloaded MoMorph exports (`public/kudos-compose/icon-*.svg`) all carry
 * a hardcoded `fill="white"` -- correct for the dark-background contexts
 * these shared icon-set instances render in elsewhere (site header, FAB),
 * but invisible against this modal's white/cream/gold surfaces when
 * loaded via `<img>` (which can't respond to CSS `color`). Inlined here
 * with `fill="currentColor"` so each usage site sets a real color via
 * `className`/`style`. `icon-image-remove.svg` stays a plain `<img>` --
 * its one usage (white X on a solid red circle) already has real
 * contrast.
 */
